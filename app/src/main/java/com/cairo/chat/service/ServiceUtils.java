...
public class ServiceUtils {
        ...
        if (!isServiceFriendChatRunning(context)) {
            ...
            connectionServiceFriendChatForStart = new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName className,
                                               IBinder service) {
                    FriendChatService.LocalBinder binder = (FriendChatService.LocalBinder) service;
                    for (Friends friends : binder.getService().friendsList.getFriendsList()) {
                        binder.getService().mapMark.put(friends.idRoom, true);
                    }
                }
    ...
    public static void updateFriendStatus(Context context, FriendsList friendsList){
        if(isNetworkConnected(context)) {
            for (Friends friends : friendsList.getFriendsList()) {
                final String fid = friends.id;
